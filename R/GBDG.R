...
#
# Log transformed analysis of UACR data from study GBDG comparing various doses of study drug against placebo.
...
vsplacebo_glargine$TRT01A <- "Dula_1.5"
...
trt_diff$TRT01A <- "Dula_1.5"
trt_diff$AVISIT <-"Change in log(UACR) vs Placebo"
names(trt_diff) <-c("geomean", "geoSE","TRT01A","AVISIT")
ci1<-data.frame(confint(fit,level=0.95))
ci1 <- data.frame(ci1[2,1:2])
ci1$TRT01A <- "Dula_1.5"
ci1$AVISIT <-"Change in log(UACR) vs Placebo"
